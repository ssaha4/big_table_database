...
public class Maputils {
	...
	public static int CompareMapWithMapForSorting(AttrType fldType, Map m1, Map m2, int map_fld_no)
		...
		case 7: 
			...
			try {
				int rowCmp = m1.getRowLabel().compareTo(m2.getRowLabel()); //when equal compareTo gives 0
				if (rowCmp == 0) {
					return 0;
				}
				if (rowCmp < 0) {
		...
		case 10:
			try {
				...
				int valCmp = m1.getValue().compareTo(m2.getValue()); //when equal compareTo gives 0
				if (valCmp == 0) {
					return 0;
				}
				if (valCmp < 0) {
		...
		case 6:
		{
			String m1_s = m1.getValue();
			String m2_s = m2.getValue();
